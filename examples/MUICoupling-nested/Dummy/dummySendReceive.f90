...
program main
  ...
  real(c_double) :: tolerance=1e-16_c_double

  real(mytype) :: point_x,point_y,point_z,point_x_2,point_y_2,point_z_2
  integer(kind=8) :: i,j,k, itime,ntime
  real(mytype) :: T,dt,zlz,yly,dz,fetchedVal
  real(mytype), dimension(ny,nz) :: bxx1,bxy1,bxz1
  ...
  do itime = 1, ntime
    ...
    do k = 1, nz
      do j=1, ny
        ...
        point_x_2 = 50.0_mytype
        point_y_2 = 20.000000000000011_mytype
        point_z_2 = 0.0_mytype
    ...
    call mui_fetch_exact_exact_3d_f(uniface_pointers_3d(1)%ptr, "ux"//c_null_char, point_x_2, point_y_2, &
         point_z_2, t, spatial_sampler, temporal_sampler, fetchedVal)
    

         write(*,*) "At ",ntime, point_x_2, point_y_2, point_z_2, "Dummy received values", fetchedVal
  enddo

  deallocate(uniface_pointers_3d)

  write(*,*) "Dummy Finalized !!!!"
